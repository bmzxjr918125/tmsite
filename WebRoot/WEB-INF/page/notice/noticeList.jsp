		...
		<div class="centercontent">
			<div id="contentwrapper" class="contentwrapper">
				...
				<div class="tableoptions">
					...
					<input class="searchinput" type="text" id="notice_title" style="width: 200px;"/>
					
					<a class="query_button btn btn_orange btn_search radius120" id="operator_query_button" style="cursor: pointer;margin-left: 50px;" >
						<span>查询</span>
					</a>
					
					<a class="query_button btn btn_orange btn_home radius120" href="admin/noticeAddNotice" style="cursor: pointer;margin-left: 50px;" >
						<span>新增</span>
					</a>
				</div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		var columns = [
			...
			},
			{
				...
				"data":null,
		    	"render":function(data,type,full){
		    		var content = "<a class='stdbtn orange' style='font-size: 12px;' href='admin/noticeUpdateNotice?noticeId="+ full.id +"'>编辑</a>";
		    		
		    		content = content + "&nbsp;&nbsp;<a class='stdbtn orange' style='font-size: 12px;' onclick='deleteNotice("+ full.id +")'>删除</a>";
		    		
		    		if(full.status == 1){
		    			content = content + "&nbsp;&nbsp;<a class='stdbtn orange' style='font-size: 12px;' onclick='closeNotice("+ full.id +",-1)'>下架</a>";
		    		}else if(full.status == -1){
		    			content = content + "&nbsp;&nbsp;<a class='stdbtn orange' style='font-size: 12px;' onclick='closeNotice("+ full.id +",1)'>上架</a>";
					}
		...
		oTable = DataTablePack.serverTable(table, "admin/noticeAjaxShow", reqData, columns, order, options);
		
	});
	
	jQuery("#operator_query_button").click(function() {
		reqData["notice_title"] = jQuery("#notice_title").val();
		oTable.fnDraw();
	});
	...
	*/
	function deleteNotice(id){
		...
		jConfirm("<span style='color:red;font-size:16px;margin-left:50px;'>确定是否删除公告，删除后将不能恢复？</span>","删除公告", function(r){
			
			if (r) {
				
				var url = "admin/noticeDeleteNotice?noticeId=" + id;
	...
	* 下架，上架
	*/
	function closeNotice(id,status){
		...
		jConfirm("<span style='color:red;font-size:16px;margin-left:50px;'>"+html1+"</span>",html2, function(r){
			...
			if (r) {
				...
				var url = "admin/noticeCloseNotice?noticeId=" + id + "&status=" + status;
